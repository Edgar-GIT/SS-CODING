...
export function GuidesPage() {
  ...
  const filteredGuides = useMemo(() => {
    ...
    return guides.filter((guide) => {
      const searchableValues = [guide.name, guide.id, ...guide.aliases].map(normalize);
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="relative min-h-[calc(100vh-3.5rem)] overflow-hidden">
        ...
        <div
          ...
          style={{
            background:
              "radial-gradient(circle at 26% 0%, oklch(0.42 0.21 285 / 0.34), transparent 34%), radial-gradient(circle at 62% 46%, oklch(0.27 0.16 235 / 0.22), transparent 42%), linear-gradient(180deg, oklch(0.13 0.05 270), oklch(0.1 0.04 260))",
          }}
        />
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Search guides by language..."
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  >
                    <img
                      src={guide.logo}
                      alt={`${guide.name} logo`}
                      className={`object-contain ${guide.logoClassName ?? "h-8 w-8"}`}
                    />
                  </div>

                  <span className="mt-4 inline-flex h-6 shrink-0 items-center gap-2 rounded-full border border-accent/40 bg-accent/10 px-3 text-[10px] font-semibold uppercase text-accent">
                    <span className="h-1.5 w-1.5 rounded-full bg-accent" />
                    {guide.status}
                  </span>
                </div>

                <h2 className="mt-6 font-display text-2xl font-bold leading-tight tracking-normal text-foreground">
                  {guide.name}
                </h2>
                <p className="mt-2 text-sm text-muted-foreground">{guide.chapters} chapters</p>
              </article>
            ))}
          </div>
